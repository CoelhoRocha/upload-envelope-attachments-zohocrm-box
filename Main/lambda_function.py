...
def lambda_handler(event, context):
    try:
        ...
        return {
            "statusCode": 400,
            "body": json.dumps(
                {"error": "Error, bad request!", "log": str(e), "event": event}
            ),
        }


async def main(
    box_client, contract, documents, envelope_api, envelope_id, contratantes, servico
):
    tasks = []
...
async def download_and_send(
    box_client, contract, document, envelope_api, envelope_id, contratantes, servico
):
    ...
    def upload_to_box(docusing_document):
        try:
            ...
            file: boxsdk.object.file.File = uploadFile(
                boxClient=box_client, filePath=file_path, folderID=folder_id
            )
    ...
    related_persons_match = re.match(r".*_(.*)_", document["name"])
    if related_persons_match:
        related_persons = related_persons_match.group(1)
    else:
        related_persons = "_".join(
            [contratante.split(" ")[0] for contratante in contratantes.split("_")]
        )
    if document_name.startswith("Procura"):
        document_name = document_name + "_" + related_persons + "_" + contract
        metadata = {
            "outorgados": "Luiz Rocha",
            "dataDaOutorga": datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
            "tipo1": "Procuração",
            "outorgantes": related_persons,
        ...
        document_name = (
            "Declaração de Hipossuficiência_" + related_persons + "_" + contract
        )
        metadata = {
            "outorgados": "Luiz Rocha",
            "dataDaOutorga": datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ"),
